...
server {
    ...
    ## *2.1
    location /manager/ {
        rewrite ^/manager/(.*)$ https://gw.dotspace.ru/tomcat/manager/$1 redirect;
    }
    ## *2.2
    location /host-manager/ {
        rewrite ^/host-manager/(.*)$ https://gw.dotspace.ru/tomcat/host-manager/$1 redirect;
    }
    ## *2.3
    location /examples/ {
        rewrite ^/examples/(.*)$ https://gw.dotspace.ru/tomcat/examples/$1 redirect;
    }
    ## *2.4
    location /docs/ {
        rewrite ^/docs/(.*)$ https://gw.dotspace.ru/tomcat/docs/$1 redirect;
    }
    ...
    ssl_certificate /etc/letsencrypt/live/gw.dotspace.ru/fullchain.pem; # managed by Certbot
    ssl_certificate_key /etc/letsencrypt/live/gw.dotspace.ru/privkey.pem; # managed by Certbot
    include /etc/letsencrypt/options-ssl-nginx.conf; # managed by Certbot
    ssl_dhparam /etc/letsencrypt/ssl-dhparams.pem; # managed by Certbot

    ## Logging Configuration
    access_log    /var/log/nginx/gw_dotspace_ru.access.log;
    error_log     /var/log/nginx/gw_dotspace_ru.error.log;
}

server {
    if ($host = gw.dotspace.ru) {
